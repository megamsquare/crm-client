...
class ResponseHandlerService
{
    ...
    public function __construct($response)
    {
        if (is_null($response) || !is_array($response)) {
            ...
            if (isset($response[self::P_STATUS])) {
                ...
                ResponseHandlerService::$lastStatus = $response[self::P_STATUS];
                if ($this->status == self::STATUS_OK) {
                    RequestHelperService::setAccessToken(isset($response[self::P_ACCESS_TOKEN]) ? $response[self::P_ACCESS_TOKEN] : null);
    ...
    {
        return ResponseHandlerService::$lastStatus == self::STATUS_LOGIN_REQUIRED || ResponseHandlerService::$lastStatus == self::STATUS_ACCESS_DENIED;
    }
